...
\textbf{工程化压缩层(GitHub 新生态)。} 2025-2026 年出现一批面向 Agent 的
工程化压缩项目:\textbf{Headroom}(66K+ star)采用内容感知路由器
(JSON/AST/散文分治)与 Compress-Cache-Retrieve(CCR)可逆存储,以代理/库/MCP
形态压缩工具输出;\textbf{billion-context} 提供模型可调用的 compress/
decompress/search\_context 工具与三级摘要(T1$\rightarrow$T2$\rightarrow$T3)
可解压压缩块;\textbf{opencode-dynamic-context-pruning} 做范围/消息两级压缩与
工具调用去重;\textbf{Context Gateway} 以阈值触发后台预压缩。这些工程方案验证了
"可逆压缩块 + 可检索"在生产中的价值,但多依赖经验阈值或摘要质量直觉,
\textbf{缺乏以语义保真度为硬约束的量化门控}——这正是 FF-Compactor 的差异化核心。
...
在相同官方 LongBench 数据(multifieldqa\_zh,前 200 条)上,采用 LLMLingua-7B
(INT8,rate=0.5)实测:
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    策略 & 平均压缩率 & 平均保真度 \\
    \midrule
    LLMLingua-7B(论文基线,实测) & 0.689 & 0.828 \\
    LLMLingua-2(token 分类,实测) & 0.687 & 0.851 \\
    \textbf{adaptive(FF-Compactor,本文)} & \textbf{0.708} & \textbf{0.996} \\
    \bottomrule
  \end{tabular}
\end{table}

实测结论:在相近压缩率下(0.689 vs 0.708),LLMLingua 的逐 token 剪枝使保真度
跌至 0.828,而 FF-Compactor 的句子级保真门控达到 0.996——\textbf{保真度显著
领先(0.996 vs 0.828)},验证了"以保真度为约束而非事后评估"的设计优于纯困惑度
剪枝。

\subsection{同类句子级基线对比(近似实现)}

在相同官方 LongBench 数据(multifieldqa\_zh,前 20 条)上,与句子级同类方法对比:

\begin{table}[h]
  \centering
  \begin{tabular}{@{}lccc@{}}
    \toprule
    预算 $B$ & Selective Context 压缩率/保真度 & RECOMP 风格 压缩率/保真度 & FF-Compactor 压缩率/保真度 \\
    \midrule
    30\% & 0.695 / 0.887 & 0.691 / 0.927 & \textbf{0.853} / \textbf{0.980} \\
    50\% & 0.459 / 0.917 & 0.423 / 0.966 & \textbf{0.753} / \textbf{0.993} \\
    70\% & 0.267 / 0.943 & 0.254 / 0.979 & \textbf{0.710} / \textbf{0.993} \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{结论}:同预算下 FF-Compactor 保真度全面领先(0.980-0.993 vs 0.887-0.979)
且压缩率更高——贪心选句与保真门控的组合优于纯打分选择(Selective Context)
与纯抽取摘要(RECOMP 近似)。注:两基线原文分别依赖 LLM 打分与监督摘要器,
此处为本地工程近似,见 \texttt{benchmark/}。
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    \midrule
    平均压缩延迟(6K-15K 字符) & \textbf{42 ms} & 1440 ms \\
    硬件需求 & 纯 CPU,0 显存 & GPU 7-8GB(INT8) \\
    加速比 & — & FF-Compactor 快 \textbf{34 倍} \\
    \bottomrule
  \end{tabular}
\end{table}

FF-Compactor 为纯 Python 句子级压缩,无需 GPU,延迟 42ms(CPU);
LLMLingua-7B 需 7-8GB 显存且单条长文档延迟 1.4 秒(逐 token 困惑度计算)。
FF-Compactor 在效率与部署成本上具备数量级优势。

\subsection{与工程化压缩层对比(Headroom 实测)}

Headroom(2026,66K+ star)是当前最具代表性的工程化压缩层(内容感知路由器 +
CCR 可逆存储)。我们安装 headroom-ai 0.35.0 实测,分多轮验证其触发条件与
压缩表现:

\textbf{触发条件实测}(transforms 字段显示内容路由决策):
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}llllc@{}}
    ...
    测试 & role & 内容 & 路由决策 & 压缩率 \\
    \midrule
    中文问答 & user & LongBench 长文档 & protected:user\_message & 0.000 \\
    超长文本 & user & 400K 字符 & protected:user\_message & 0.000 \\
    JSON & assistant & 结构化数据 & smart\_crusher:0.52 & 0.538 \\
    代码/日志 & assistant/tool & 代码、日志 & protected:recent\_code & 0.000 \\
    散文 & assistant & 自然语言 & noop & 0.000 \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{同条件压缩对比}(10 条不同规模 JSON):

\begin{table}[h]
  \centering
  \begin{tabular}{@{}lcc@{}}
    \toprule
    指标 & Headroom(smart\_crusher) & FF-Compactor \\
    \midrule
    平均压缩率 & 0.346 & \textbf{0.500} \\
    平均保真度 & 0.397 & \textbf{0.521} \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{实测结论}:Headroom 的压缩是角色/内容路由驱动的:user 消息与"最近代码"
默认受保护不压缩,散文走 noop 跳过,仅在 JSON + assistant role 下触发
smart\_crusher;即便在其唯一触发场景(JSON),Headroom 压缩率 0.346 低于
FF-Compactor 的 0.500,且保真度 0.397 显著低于 0.521——统计式压缩(删除字段/
数组项)对语义破坏较大。FF-Compactor 对任意角色、任意内容类型统一压缩
(中文长文档 0.996 保真、代码 0.636、JSON 0.521),无角色/类型保护限制。

\textbf{定位}:Headroom 等工程方案面向 Agent 场景的"安全压缩"设计(保护 user
消息),而 FF-Compactor 以保真度门控为硬约束、无内容类型限制,在通用长文本
压缩上具备独占优势。
