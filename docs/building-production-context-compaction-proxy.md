...
在 **THUDM/LongBench 官方数据集**(30 子集 / 340 样本,真实 6K-15K 字符长文档)上验证:

| 策略 | 平均压缩率 | 平均语义保真度 | 样本数 |
|------|-----------|---------------|--------|
...
**实测结论**:在官方真实长文档上,adaptive 实现 **70.8% 压缩率 + 99.6% 语义保真度**——压缩率与 SOTA 基线 LLMLingua-7B 相当(0.708 vs 0.689),但**保真度领先 17 个百分点**(0.996 vs 0.828),且**快 34 倍**(42ms CPU vs 1440ms GPU)。对比工程化压缩层 **Headroom**(66K+ star):其压缩由内容路由驱动,默认保护 user 消息与"最近代码"、散文走 noop,仅在 JSON+assistant 下触发 smart_crusher(同批 JSON 实测 0.346 压缩/0.397 保真,FF-Compactor 0.500/0.521 全面领先)。相比粗暴摘要(0.982 压缩 / 0.796 保真),adaptive 保真度高出 20 个百分点。这印证了保真度底线设计的核心价值:**压缩不是越狠越好,信息完整才是底线**。完整数据(消融 / 灵敏度 / 效率 / Q&A)见 [BENCHMARK.md](../BENCHMARK.md)。
