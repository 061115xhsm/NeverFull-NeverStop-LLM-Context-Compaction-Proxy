...
**实测结论**:在官方真实长文档上,adaptive 实现 **70.8% 压缩率 + 99.6% 语义保真度**——压缩率与 SOTA 基线 LLMLingua-7B 相当(0.708 vs 0.689),但**保真度领先 17 个百分点**(0.996 vs 0.828),且**快 34 倍**(42ms CPU vs 1440ms GPU)。相比粗暴摘要(0.982 压缩 / 0.796 保真),adaptive 保真度高出 20 个百分点。这印证了保真度底线设计的核心价值:**压缩不是越狠越好,信息完整才是底线**。完整数据(消融 / 灵敏度 / 效率 / Q&A)见 [BENCHMARK.md](../BENCHMARK.md)。
